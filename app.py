...
if categoria == "Formato Creación Usuarios":
    ...
    if submit_u:
        try:
            ...
            # --- CORRECCIÓN 1: RELLENO EN COLUMNA B (CELDAS DE RESPUESTA REALES) ---
            ...
            # --- CORRECCIÓN 2: LOCALIZACIÓN INTELIGENTE DE CABECERAS ---
            fila_inicio_tiendas = None
            fila_inicio_vendedores = None
            
            for row in range(1, 100):
                val_col_c = str(ws.cell(row=row, column=3).value).strip().upper() # Columna C ("DIRECCION")
                val_col_b = str(ws.cell(row=row, column=2).value).strip().upper() # Columna B ("NOMBRE TIENDA")
                
                if "DIRECCION" in val_col_c and fila_inicio_tiendas is None:
                    fila_inicio_tiendas = row + 1 # La escritura empieza en la fila siguiente
                elif "NOMBRE VENDEDOR" in val_col_c and fila_inicio_vendedores is None:
                    fila_inicio_vendedores = row + 1
                    
            # Si la búsqueda automática falla por algún motivo, usamos los valores por defecto seguros
            if not fila_inicio_tiendas: fila_inicio_tiendas = 22
            if not fila_inicio_vendedores: fila_inicio_vendedores = 42

            # --- CORRECCIÓN 3: ESCRITURA RESPETANDO LAS COLUMNAS EXACTAS (B, C, D, E) ---
            # Insertar tiendas
